...
def F(x,y,E):
    ...
    for n in range(14):
        ...
        E2.append(I2)
        E3.append(I3)
    H1 = V * (x[14] + x[13]) - chi * y[14] * x[14] + E * x[14]  # * (0+1j))
    H2 = 0.6 * x[14] ** 2 - 0.356 * (-1 + sp.exp(-4.45 * y[14])) * sp.exp(-4.45 * y[14]) + (-0.02 * y[0] + 0.02 * y[14]) * sp.exp(
        -0.35 * y[0] - 0.35 * y[14]) - 0.0035 * (y[0] - y[14]) ** 2 * sp.exp(-0.35 * y[0] - 0.35 * y[14]) - 0.0035 * (
                     -y[13] + y[14]) ** 2 * sp.exp(-0.35 * y[13] - 0.35 * y[14]) + (-0.02 * y[13] + 0.02 * y[14]) * sp.exp(
        -0.35 * y[13] - 0.35 * y[14])
    H3 = x[14]**2
    E1.append(H1)
    E2.append(H2)
...
H = sp.Inverse(B2)
print('H=', np.shape(sp.Matrix(H)))
#print('f shape=', np.shape(F(x,y,E)))
#print('f t shape=', np.shape(np.transpose(sp.Matrix(F(x,y,E)))))
#print('multiplication is =', np.shape((np.transpose(sp.Matrix(F(x_0,y_0,E_0))))* sp.Matrix(H)))
...
#B1[:, :, 0] = B
#print('B1=',np.shape(B1[:,:,0]))
#print('B1 matrix =',np.shape(sp.Matrix(B1[:,:,0])))

for k in range(20):
    print('k=', k)
    print('4=', ((sp.Matrix(np.transpose(sp.Matrix(-F(x,y,E)))))* sp.Matrix(sp.Inverse(sp.Matrix(B)))).as_coeff_mul())
    #print('4=', ((sp.Matrix(np.transpose(sp.Matrix(-F(l[:, k], m[:, k], o[:, k])))))* sp.Matrix(sp.Inverse(sp.Matrix(B1[:, :, k])))).as_coeff_mul())
    print('b=' , np.shape(b[:,k]))
    #H1[:,:, k] =  np.array(sp.Matrix(B1[:, :, k]).inv())
    b[:,k] =  ((sp.Matrix(np.transpose(sp.Matrix(-F(l[:, k], m[:, k], o[:, k])))))* sp.Matrix(sp.Inverse(sp.Matrix(B1[:, :, k]))))
    t2[:, k+1]  = t2[:, k] + b[:, k]
    l[:, k+1] = t2[0:15, k+1]
    m[:, k+1] = t2[15:30, k+1]
    o[:, k+1] = t2[30, k+1]
