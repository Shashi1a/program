...
for j in range(len(gray_imgs)):
 ...
 # making a  sliding window
    ...
    sx  = pd.DataFrame(sp_x).to_csv('./xData2.csv')
    sy  = pd.DataFrame(sp_y).to_csv('./yData2.csv')
    bx = pd.read_csv('./xData2.csv')
    #cx  = pd.DataFrame(bx.loc[6]).to_csv('./6xdata.csv')
    by = pd.read_csv('./yData2.csv')
...
# and the second max and then define function A and theta and thus calculate u1 and u2
    ...
    IMF_max1 = np.max(im_IMF[0:483]); IMF_max2 = np.max(im_IMF[484:967])
...
vdf.append(df)
fdf = pd.concat(vdf).to_csv('./im_sp_data2.csv', sep=',', index=False, header=True)
