...
def DNA(t,x,y,z):
	...
	I3 = - 0.005 * z[n] + 11.4838709677419 * (-1.0 + sp.exp(-4.45 * y[n])) * sp.exp(-4.45 * y[n]) + 0.112903225806452 * ((-y[n] + y[n+1])**2) * sp.exp(-0.35 * y[n] - 0.35 * y[n+1]) + 0.112903225806452 * ((y[n] - y[n-1])**2) * sp.exp(-0.35 * y[n] - 0.35 * y[n-1]) - 32.258064516129 * (2 * y[n] - 2 * y[n+1]) * (0.01 * sp.exp(-0.35 * y[n] - 0.35 * y[n+1]) + 0.02) - 32.258064516129 * (2 * y[n] - 2 * y[n-1]) * (0.01 * sp.exp(-0.35 * y[n] - 0.35 * y[n-1]) + 0.02) - 19.3548387096774 * (np.abs(x[99])**2)
	I2 = z[n]
	E1.append(I1)
	E2.append(I2)
	E3.append(I3)
    ...
    #print('H1=', H1)
    H3 = -0.005 * z[99] + 11.4838709677419 * (-1.0 + sp.exp(-4.45 * y[99])) * sp.exp(-4.45 * y[99]) + 0.112903225806452 * ((-y[99] + y[0])**2) * sp.exp(-0.35 * y[99] - 0.35 * y[0]) + 0.112903225806452 * ((y[99] - y[98])**2) * sp.exp(-0.35 * y[99] - 0.35 * y[98]) - 32.258064516129 * (2 * y[99] - 2 * y[0]) * (0.01 * sp.exp(-0.35 * y[99] - 0.35 * y[0]) + 0.02) - 32.258064516129 * (2 * y[99] - 2 * y[98]) * (0.01 * sp.exp(-0.35 * y[99] - 0.35 * y[98]) + 0.02) - 19.3548387096774 * (np.abs(x[99])**2)
    H2 = z[99]
    E1.append(H1)
    E2.append(H2)
    E3.append(H3)
